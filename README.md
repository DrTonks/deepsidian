...
**0.5.0：同库记忆、闲时待审提案、来源范围与撤销恢复。** 已保存的记忆可以参与新会话，修改/删除从下一次提问起更新。输入 `/extract` 选择会话，生成带原文来源的新增/更正提案，勾选确认后保存。闲时生成待审提案现可在设置中开启，默认关闭。记忆系统完成前保持 0.5.x。下方历史版本段落保留当时状态，以本节为当前能力说明。
...
- 已补充有限变更历史/单次撤销、同机死进程锁恢复和“仅从现在起贡献”。原生Obsidian长期使用验收仍待进行；不支持多个插件进程并发共享同一预算账本。详见 [当前记忆实现](docs/MEMORY-DESIGN.md)。
...
新增12类付费合成评测，覆盖归属、临时要求、重复、更正、凭据与不记住。两次复核分别发现旧值混入正文和排他约束丢失，修复后第三轮12例符合预期。见 [评测说明](docs/MEMORY-EVALUATION.md)。原生开发库自动连接已观察到；闲时、多窗口、重启全流程验收尚未完成，暂不升级0.6。
