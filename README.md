...
**0.6.0：同库长期记忆与可选的 AI 记忆管理。** 保存、跨会话读取、更正、遗忘、提炼审批、闲时待审、来源控制及最近操作撤销构成记忆 v1。下方历史版本段落保留当时状态，以本节为当前能力说明。

在插件设置开启 **“允许 AI 管理记忆”**（默认关闭），即可直接说“请记住，我喜欢先看例子”“更正为先给定义”或“忘记这条偏好”。模型通过 `memory_manage` 工具保存，成功后简短反馈；无需再次审批，可在 `/memory` → 维护页撤销最近操作。需同时开启全局及本会话记忆读取、会话贡献。关闭此开关会移除写入工具，仍可手动管理。
