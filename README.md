...
**0.5.0：同库记忆读取、会话级控制与手动模型提案。** 已保存的记忆可以参与新会话，修改/删除从下一次提问起更新。输入 `/extract` 选择会话，生成带原文来源的新增/更正提案，勾选确认后保存。闲时生成待审提案现可在设置中开启，默认关闭。记忆系统完成前保持 0.5.x。下方历史版本段落保留当时状态，以本节为当前能力说明。
...
- 此版本仍需补充撤销日志、自动锁恢复和实机长期使用验收；不支持多个插件进程并发共享同一预算账本。
