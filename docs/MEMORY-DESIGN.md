...
## 当前版本：记忆 v1 功能闭环（2026-09-19，插件 0.6.1）

这是当前实现的权威摘要。下方按日期保留的阶段记录及目标设计不表示仍然缺少已交付功能，也不表示全部远期设计已实现。

本库保存/读取、更正删除、人工提炼审批、闲时待审生成、来源范围与撤销恢复已形成闭环。“允许 AI 管理记忆”自0.6.1起新安装默认开启、保留已有关闭选择：开启后，当前用户明确要求可由聊天模型通过 `memory_manage` 直接提交，不必调用 `/extract`。记忆由插件管理，DSH 提供模型运行和会话历史，不依赖其 Web profile 的记忆插件。
