...
独立分支 `codex/experimental-tab-completion` 实现 A+B：无会话的 DSH 短请求、单行灰字、Tab/Esc、单次撤销、请求归属与取消保护、调用额度及目录排除。默认关闭，不提供自动触发。付费 flash 第二轮固定20例有16条可用、3次合理无建议、1例错误前提未被识别；另一个英文拼接问题已由宿主校验拦截。后续优先真实中文输入法、Windows、第三方补全共存与实际接受率，再考虑自动模式。见 [实验记录](TAB-COMPLETION-EXPERIMENT.md)。
