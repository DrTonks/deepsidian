...
## 0.6.2：提前落地可用的笔记助手（本节覆盖上方版本安排）

按用户要求，这组笔记上下文能力仍归入 **0.6.x**，不升级0.7。

已实现：属性筛选分页、Wiki别名/标题/普通段落块定位、一跳关联候选、来源预览与手动附加、Base原始配置读取、`/catalog` 本地预览确认生成管理表格与导航。工具结果受每轮次数和字符预算约束；取消或换轮后晚到结果失效。输出表格依据 [Obsidian Bases 官方语法](https://obsidian.md/help/bases/syntax)。生成的Base为动态查询，导航为生成时快照；保留原文章路径及frontmatter。

验收：离线覆盖路径边界、正文定位、分页/截断、关联原因、Base来源、大库目录筛选、取消和预算；两种DSH协议及旧会话迁移回归；官方付费模型完成属性筛选、标题事实读取、关联理由三个合成场景。浏览器使用生产来源组件，验证定位错误、候选选择、附加片段与窄屏；这不替代原生Bases渲染验收。

当前边界：最多扫描2000篇；属性依赖缓存；不执行任意Base公式，不保证复现用户复杂表格；普通段落块以外明确报不支持；关联仅链接/标签，不能推出知识掌握程度；不批量改frontmatter，不覆盖已有管理文件。字符预算不包含DSH全部历史、网络工具和内部重试，不能当总token/费用限制。模型可能生成额外的不精确链接示例，提示词要求引用实际path，仍需持续质量评测。

后续0.6.x优先：
1. 在实际库验收生成表格、分类字段差异、重名链接和未保存编辑；积累断链样例。
2. 目录增量刷新与已有文件差异预览，保持人工确认，不直接重写博客结构。
3. 来源定位跳转、复杂块解析与元数据缓存刷新提示；按大库实测决定是否添加索引。
4. 模型参数弹窗与窄侧栏收纳、记忆历史保留/清理；DAG、会话分支与Tab补全保留为独立实验，不捆绑升级。


本轮最终验证：`npm run check` 108通过、1项Windows权限跳过，构建通过；DSH集成8/8（含rc.2迁移）；官方付费三轮各3场，最后一轮路径引用修正后事实与引用人工核对通过。目录写入5项生产流程回归覆盖预览只读、同名拒绝、部分失败、失效预览、预览后junction替换。已安装开发库0.6.2；原生Base表格渲染仍待使用者验收。

## 0.6.3：目录差异更新与来源跳转（2026-09-20）

已实现 `/catalog` 重新扫描及新增/移除/标题与分类变化预览，确认后仅更新导航生成区域；已有 Base、自定义前后文、原文章保持不变。仍是最多2000篇的有界重扫，不是增量索引；改名显示为旧路径移除与新路径新增。新版导航内保存生成记录与内容校验，生成区被手改、预览后发生编辑、存在未保存导航或范围不同均拒绝覆盖。0.6.2及更早导航没有可信生成记录，保留原文件并选择新输出目录；不自动迁移手写内容。

来源预览增加“跳转原文”，回答 Wiki 引用使用提问时的来源路径解析；点击时重新检查文件和标题/普通块，断链不创建新笔记。重名解析沿用 Obsidian 按来源选择目标的规则，并返回实际路径；未保存正文优先读活动编辑器。属性筛选继续依赖元数据缓存，工具结果明确提示可能滞后。导航兼容 category 数组及 categories。

验证：115离线、8集成、3个真实flash知识工具场景通过；浏览器来源交互通过；macOS Obsidian 1.13.7 实机验证目录生成→改名/改分类→差异确认→更新，并校验 Base 字节不变、手写前后文保留；来源预览和真实回答引用可点击到标题。

### 0.6.4 实现前的分支讨论（历史记录）

本机 DSH 0.1.6-alpha.2 的 SessionStore.fork 支持指定事件边界，API session/fork 接受 sessionId 与 atSeq。边界须为已闭合回合；仅调用内存 store 不等于已有持久化生命周期，因此仍需验证 sdk-minimal 中的桥接与重启恢复，尚未实现插件分支功能。

建议首版：已完成回答下提供“分支到新聊天”；子会话继承截至该轮的真实运行时历史，独立续聊，显示来源会话及分支点。需补持久消息/回合与DSH事件序号的对应，旧聊天缺少可靠映射时不能靠前端数组猜测。首版不合并分支、不复制个人笔记库；长期记忆仍在同一库共享，但继承的用户消息不应重复贡献记忆提炼。附件可读性、父子独立写入、取消与重启恢复均需端到端验证。

## 0.6.4：从完成回答创建持久分支（2026-09-20）

已实现回答下“分支到新聊天”、父对话/分支点展示及返回。回答记录对应的 DSH `turn/end` 序号，桥接读取该精确边界的持久事件前缀，通过 `agents.create(seed, inheritedEventCount, meta.parentSession)` 创建子会话并等待落盘，之后才发布插件聊天记录。附件引用、工具结果和递归分支中的继承事件序号保持可用。旧回答无映射时拒绝分支；未完成、停止、失败的回答也拒绝。保存失败保留原对话，可能留下未出现在插件列表的 DSH 子会话日志，不自动删除运行时记录。

分支继承目标及会话记忆开关；同库记忆仍共享。继承消息的哈希与长度构成记忆来源下限，即使选择“全部消息”也不重复提炼继承历史。分支中新产生的消息可以贡献记忆。分支不隔离工具对共享记忆的修改，不冻结或复制笔记文件；历史工具结果属于当时快照，新读取来自当前库。创建分支不产生模型请求，续聊才产生请求；继承历史仍可能占用输入 token，不等于免费上下文。

验证：117 离线测试、8 DSH 集成测试通过，类型检查与构建通过；集成覆盖精确截断、工具/图片继承、父子独立续聊、递归分支、重启恢复、重复ID/错误边界拒绝及子会话取消。官方付费 `deepseek-flash` 合成验收 6/6，macOS Obsidian 1.13.7 实机完成新回答→分支→⌘ Enter续聊→返回父对话。

### 0.6.5 最初提案（历史记录，回传部分已取消）

优先补 P1b 上下文清单，并完成 P2 的“分支结论回到主线”最小闭环：

1. 在发送前列出本轮显式附加的笔记、选区、附件与记忆开关；标明继承历史仍保留哪些旧资料，避免把“移除下一轮附件”说成“从历史中抹去”。每项保留来源路径/位置、内容版本或哈希及字符数，无法精确取得 token 时不伪装为精确计费。
2. 在分支中手动选取结论，预览文字与来源，再附加到原对话的输入草稿。用户确认发送后才成为原对话的新消息；不自动合并完整历史，不后台追加，不自动改写笔记。
3. 用“主线学习注意力→支线理解 KV cache→带回两句结论”验收：父历史不变，摘要来源能返回分支，旧资料/新资料可区分，刷新与失败不会重复追加。先做好列表及来源交互，再考虑 Canvas/DAG。

相关概念：**会话分支**是复制到某一完成轮次的历史前缀，未来分别追加；**lineage（谱系）**记录来自哪个父会话及哪个边界；**上下文**是模型这一次实际收到的材料，受窗口和预算限制，完整历史并不保证永远原样进入每次请求；**ContextManifest（上下文清单）**是这些材料的来源与版本记录；**长期记忆**是同库跨会话复用的独立存储；**摘要回传**是用户确认后新增的一条带来源消息，不是将两条事件历史自动合并。


### 2026-09-20 Windows 拉取复审

已从 b6b00be 快进至 508034e（0.6.3、0.6.4）。两位子 agent 分别复审目录/来源与持久分支；分支未见阻断问题。目录发现并修复两项：Vault.process 等待期间出现未保存编辑时，需在同步写入回调再次检查；导航仅转换为 CRLF 后应仍通过生成区校验，并保留原换行及用户前后文。增加对应回归。

本机实际 DSH 集成8/8，官方 deepseek-flash 分支付费合成6/6通过（精确边界、父子隔离、重启保持）。原生 Windows Obsidian 新分支与 Base UI 本轮未实机操作；macOS 实机证据沿用协作者文档，不混作本机验收。保留维护债务：DSH 子会话已落盘但插件索引保存失败时可能留孤立日志，后续需要恢复记录而非直接删除日志。版本保持0.6.4，0.6.5计划不变。

最终复验：另一位子agent独立复审两个修复未发现问题；全量119项中118通过、1项Windows权限跳过，类型检查/构建通过，git diff --check通过。8集成与6付费场景均通过。已安装开发库0.6.4；两项修复及记录保留本地未提交，未push。


## 0.6.5 最终范围：上下文清单与独立草稿（2026-09-21）

按用户反馈移除结论回传原型，不提供回传按钮、摘要暂存、自动拼接或子 agent 分派。Fork 保留从完成回答独立续聊、继续分支及父对话导航。历史研究表格中的“手动带回”不属于当前交付范围。

新增发送前清单：当前笔记与选区/附近段落、附件、图片字节数及记忆读取状态；可移除本轮资料。发送时保存来源名称、内容 SHA-256、字符数/字节数、历史消息数和记忆读取开关。清单是宿主显式输入记录，不是 DSH 完整上下文重放，不精确估算 token，不包括后续工具结果；这些继续查阅运行轨迹。排除本轮材料不删除历史中已发送的资料。

文字草稿按会话保存并恢复；附件按会话在侧栏内暂存，重启需重新选择。删除回传原型不清除试验期间已发送的聊天与运行时日志；遗留待回传字段不再被新请求读取。包含前轮 CRLF 编辑器比较修复。

验证：122 离线测试、类型检查及构建通过；macOS Obsidian 1.13.7 隔离验收库已安装，重载后回传入口消失，分支导航及上下文清单保留。原型回传的付费验收仅属于历史试验，不作为现版本功能宣称。
...
独立分支 `experimental-tab-completion` 实现 A+B：无会话的 DSH 短请求、单行灰字、Tab/Esc、单次撤销、请求归属与取消保护、调用额度及目录排除。默认关闭，不提供自动触发。付费 flash 第二轮固定20例有16条可用、3次合理无建议、1例错误前提未被识别；另一个英文拼接问题已由宿主校验拦截。后续优先真实中文输入法、Windows、第三方补全共存与实际接受率，再考虑自动模式。见 [实验记录](TAB-COMPLETION-EXPERIMENT.md)。

## 0.6.6 当前笔记学习交互（2026-09-22）

基线0.6.5，按用户决定取消分支结论回传。交付右键/命令选区提问、会话内持久选区快照、标题/行号/修订信息、来源版本变化提示；已有草稿保留，准备阶段不发模型请求。按需关联沿用related→resolve工具链，提示词要求先解决选区问题，片段不足时再找候选并说明理由，不把关联当成已读/已掌握。

边界：标题上下文只提取ATX（#）标题；手动来源快照记录全文哈希，普通旧记录和任意历史工具引用不保证都有修订信息。变化后的来源打开最新正文，无法可靠重定位时不使用旧行号；复杂块解析继续沿用原边界。没有自动写笔记、自动归类、分支结论回传或Tab补全。

下一步仍0.6.x：用实际学习流程收集选区内容不足、重复标题、跨文件问题样例；完善可解释的关联候选排序和检索质量评测；再评估复杂块及引用追踪。Canvas/DAG只保留后续探索，不先把资料图谱和对话树混为一体。


验收结果：132离线通过、1项Windows符号链接权限跳过；类型检查、构建及8项DSH集成通过。官方付费合成复验3/3，通过片段足够零工具、证据不足related→resolve、资料中记忆指令不执行三个场景，另有人工作答复核；首次测试正则误判保留原报告。生产侧栏组件的learning-tests通过，480px选区预览无页面横向溢出。原生Windows右键/快捷键本轮未实机验证，不用浏览器截图替代。子agent审查修复草稿并发覆盖、快照移除持久化、新会话来源残留及busy操作；最终复审无阻断问题。

### 0.6.6 macOS 拉取复审（2026-09-22）

已将 main 快进至 2e557c7，保留独立 Tab 补全实验分支。三位子 agent 审查后修复会话切换期间选区提问覆盖目标草稿、LF/CRLF 来源修订误判、选区发送后无编辑器时重复附带旧快照三项问题。来源修订采用带 sha256-lf: 前缀的逻辑换行哈希，兼容旧版原始哈希；发送清单指纹继续描述精确快照。

最终 138 项离线测试、类型检查、构建及生产侧栏组件回归通过；8 项 DSH 集成通过。付费 deepseek-flash 合成脚本 3/3，并检查完整回答；macOS Obsidian 1.13.7 实机验证右键提问、草稿保留、快照重载恢复、关闭笔记后发送与已消费快照清理，真实回答命中合成阈值和标记。另一位子 agent 独立复核修复无新增确定问题。详细边界见 MACOS-VALIDATION.md；本轮未验证 Windows 原生界面。代码未提交或推送。

### DSH 0.1.7 兼容基线与下一步（2026-09-26）

运行时基线更新为 0.1.7-rc.2（npm next），适配供应商配置从 settings-file 转为显式 patch、官方 Messages-only 及禁用状态覆盖。139 项离线与 11 项集成全部通过，含两个旧版历史迁移；两位子 agent 审查并完成修复复核。官方 deepseek-flash 选区/分支/记忆共 17 个合成场景通过，最终全局安装另重跑选区 3 场；原生 macOS 旧会话续聊通过。详情及边界见 MACOS-VALIDATION.md。

建议下一步 0.6.7 聚焦“检索质量评测与关联排序”：先建立固定的合成问题集及预期证据小节，包含片段足够无需检索、证据跨文件、重名标题、无关同标签和断链，再依据漏检/误检结果调整候选排序。记录来源命中、实际读取范围、回答引用正确性、延迟及供应商报告的 token，不先引入向量数据库或扩大整库上下文。

检索评测是用固定问题和预期证据衡量改动是否有进步；可解释排序是在候选旁说明链接方向、共同标签等实际依据，方便使用者判断为何推荐。先建立基准，再修改排序，并复用付费供应商回归，避免用单次“看起来回答不错”替代可重复的验证。此项为下一步建议，尚未实现。
...
### PR #1 合并后发布准备（2026-09-26）

main 已包含默认关闭的手动补全实验。社区发布前优先解决安装与真实库体验：修复三文件安装缺桥接、Astro published 日期漏识别；158 项离线与16项运行时集成通过，真实 content 工作区验证首次连接、选区、分支、引用、灰字/撤销及27篇文章Base日期。原103文件不变，测试文件归档。付费补全留出集仍有1/8矛盾续写，保持实验定位。现有文章外发范围与新显示名称待用户决定，未正式发布；详情见 MACOS-VALIDATION.md 和 COMMUNITY-RELEASE.md。后续检索质量评测与排序的计划不变，版本号在本次发布范围确定后再安排。
