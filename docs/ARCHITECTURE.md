...
`onload` 加载插件状态并注册视图、命令、事件和回收计时器。0.4.0 默认在 workspace.onLayoutReady 后异步连接 DSH，可通过 autoConnect 关闭；缺少环境时记录状态，不阻塞 Obsidian，也不弹出循环引导。`view.onOpen` 本身不连接。显式连接、模型刷新和发送也走同一连接入口；宿主将并发连接合并，并让新连接等待断开完成。闲置约五分钟执行 stop；活动请求不回收。卸载阻止迟到的自动连接，并停止已启动进程。

参数变更不修改正在运行的一轮。空闲时释放进程，下次以同一个 sessionId 恢复，并使用新 route。模型目录来自 DSH 元数据；不等于账号授权列表。

## 数据兼容

同库记忆的落地范围和后续契约见 [长期记忆技术设计](MEMORY-DESIGN.md)。0.4.0 提供手动存储/编辑和本地索引整理，尚不接入模型或后台。不能按目前 main.handleTool 的前台 busy 状态直接接入后台整理。

`commands.ts` 定义宿主斜杠菜单；view.ts 在发送前分流，管理命令不进入模型请求。`/plan` 生成一次规划请求；`/goal` 持久保存本聊天目标，后续输入携带它，不启用 DSH 原生 goal driver 或自动续跑。原生 DSH 命令需另行接入命令 registry、计划审阅与目标生命周期。
...
当前已验证 DSH 0.1.6-alpha.2（2026-09-19），同时覆盖旧 Chat Completions 与默认 Messages 路由。上游 developer preview 可能破坏接口；升级先核对包版本与新增能力，在分支运行集成测试，再更新 versions.ts 中的 TESTED_DSH 和 README；引导、诊断与 CI 共用该版本。不要只依赖 SDK initialize 中的 serverInfo.version。
...
0.6增加manageMemory设置（0.6.1起新安装默认开启，保留已有关闭选择）：桥接按开关注册memory_manage；主进程按当前轮manager身份、读/贡献权限、逐字来源和revision重新验证。MemoryManager串行处理当前轮最多5次变更，store.update同锁返回提交快照供后续读取。整理器仍无工具。权限检查在WAL事务开始前完成，已开始的提交不随取消回滚。
