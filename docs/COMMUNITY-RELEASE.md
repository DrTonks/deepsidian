...
截至 2026-09-26，PR #1 已合并至 main（30a42b1）。合并不是社区上架；首次发布还需要 GitHub Release 和 Obsidian Community 的审核。GitHub 仓库名称及简介已更新为 Deepseedian。维护者已批准提交、推送及发布 0.6.6；本轮执行范围为下方第 1–3 步，停在社区提交之前。公开发行状态可在 [GitHub Releases](https://github.com/DrTonks/deepseedian/releases) 查看。
...
- 保留插件版本 0.6.6；首次发布可使用这一版本。以后发布必须递增，Git 标签与 manifest.version 完全一致，例如 `0.6.6`，不要加 `v`。
...
- Tab 补全仍是默认关闭的实验功能。实际供应商可能返回错误事实或矛盾续写；不能将 HTTP 成功或出现灰字作为质量通过。
...
发布目录为 `dist/release/0.6.6/`，只上传其中三个标准文件。验证应从这三个文件安装到测试库，不能用保留旧 bridge.mjs 的开发目录证明首次安装正常。不要上传 data.json、.runtime、记忆、个人库、凭据或 .runs。

## 正式发布步骤

### 1. 维护者完成最终审查

先检查 `git diff`，确认代码、README、manifest.json、versions.json 与构建内容一致。重点复查模型数据流、默认开启的网络/记忆权限、补全实验说明、最低 Obsidian 版本和测试边界。测试使用合成资料即可，社区发布不需要提交个人文章。
...
在准备发布的提交上执行前文检查及 `npm run package:release`。确认 `dist/release/0.6.6/manifest.json` 与根目录 manifest 一致，在干净测试库仅安装三个文件验证。保存本次验收结果；不得把此前构建的测试结果套用到后续未验证的修改。

若提交前调整版本，同步修改 package.json、package-lock.json、manifest.json 和 versions.json，再构建。社区版本须为 `x.y.z`，初步构建不代表必须使用 `1.0.0`。

### 3. 创建 GitHub Release

审查通过后，在 [仓库 Releases](https://github.com/DrTonks/deepseedian/releases) 选择新建 Release。选择已审查的提交，创建与 manifest.version **完全一致**的标签，例如 `0.6.6`（不要加 `v`）。可以先保存草稿检查附件；供社区安装的版本最终需要公开发布。

分别上传：

- `dist/release/0.6.6/main.js`
- `dist/release/0.6.6/manifest.json`
- `dist/release/0.6.6/styles.css`

不要只上传 ZIP，GitHub 自动生成的源码压缩包也不能代替这三个附件。Release 标题可为 `Deepseedian 0.6.6`，说明当前能力、Node/DSH 前置环境、实验补全状态、费用与数据流以及验证范围。
...
当前官方流程不是向 obsidian-releases 的 community-plugins.json 提交 PR。GitHub Release 可以供手动安装，但不等于通过社区审核。本轮获准创建标签与公开 Release，但不提交社区条目；第 4 步及之后留待维护者另行推进。
