...
export class KnowledgeTools {
  ...
  async handle(name:string,args:Record<string,unknown>,sourcePath:string):Promise<unknown> {
    ...
    if(name==='obsidian_resolve') {
      ...
      return {path:file.path,subpath,revision:createHash('sha256').update(text).digest('hex'),resolvedFrom:source,notice:'按来源笔记解析链接；重名时采用 Obsidian 的解析结果，请核对返回路径。片段来自当前打开的编辑器或磁盘，不使用旧行号。',...linkedExcerpt(text,subpath)};
    }
