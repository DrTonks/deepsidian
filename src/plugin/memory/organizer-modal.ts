...
export class OrganizerModal extends Modal {
  ...
  constructor(readonly plugin:Deepsidian){super(plugin.app);this.chatId=plugin.chat?.id??'';}
  ...
  private async save(){
    ...
    try {await this.plugin.applyMemoryProposals(this.batch,[...this.selected]);this.committed=true;this.message='所选提案已保存到本库，下次提问生效。';}
    catch(error){this.message=String(error);}
    finally{this.saving=false;this.render();}
  }
  private render(){
    if(this.closed)return;
    const root=this.contentEl;root.empty();root.createEl('h2',{text:'从会话提炼记忆'});
    root.createEl('p',{text:'仅从所选会话最近 20 条用户消息中排除已遗忘来源后，连同现有记忆和整理规则发送给当前模型；不会向前补齐，不发送附件、笔记上下文或助手回答。会产生模型调用费用，生成后不会自动保存。'});
    ...
    select.value=this.chatId;select.disabled=!!this.controller||this.saving;
    ...
    this.button(root,this.batch?'重新生成提案':'生成提案',()=>void this.generate(),!!this.controller||this.saving||chat?.contributeMemory===false||!chat);
    ...
    root.createEl('small',{text:'关闭窗口会取消生成并丢弃未提交提案。模型整理日志保存在插件的 .memory-runtime 中；取消不会擦除已发送给供应商的数据。/organize 仍只重建本地索引。'});
  }
}
