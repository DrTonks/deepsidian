...
import { contributionSources, extractionPrompt, parseProposals, sourceKey, type ProposalBatch, type Evidence } from './proposals.ts';
...
export class MemoryScheduler {
  ...
  private async execute(controller:AbortController){
    ...
    try {
      ...
      let chosen:Chat|undefined,sources:Evidence[]=[],prompt='',oversized=false,invalidRange=false;
      for(const candidate of this.host.chats()) {
        ...
        try {sources=contributionSources(candidate);}catch{invalidRange=true;continue;}
        sources=sources.filter(s=>s.index>after)
        ...
        if(sources.length){chosen={id:candidate.id,title:candidate.title,messages:[],memoryPolicyVersion:candidate.memoryPolicyVersion};break;}
      }
      if(!chosen){
        const message=invalidRange?'部分会话的记忆来源起点已失效，已暂停其提炼；请在本会话重新设置来源范围。':'部分会话的单条消息或记忆/规则超出输入上限，已暂缓；请缩短内容后重试。';
        if((oversized || invalidRange) && this.host.state().lastError!==message)await this.host.save(s=>{s.lastError=message;});
        return;
      }
      ...
      const batch:ProposalBatch={chatId:chat.id,title:chat.title,snapshot,sources,proposals,memoryPolicyVersion:chat.memoryPolicyVersion??0};
      await this.host.save(current=>{
        this.valid(signal);
        ...
        if((live.memoryPolicyVersion??0)!==(chat.memoryPolicyVersion??0) || sources.some(s=>!contributionSources(live).some(current=>current.key===s.key)))throw Error('来源范围已改变');
