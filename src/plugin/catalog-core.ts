...
export function buildCatalog(articles:CatalogArticle[],source:string,output:string):CatalogPlan {
  ...
  const formulas:Record<string,string>={article:'file.asLink(if(title, title, file.name))',category:'if(category, category, if(categories, categories, "未分类"))',published:'if(note.published, note.published, if(date, date, pubDate))'};
  ...
  for(const row of rows){
    ...
    if(!present(fm.published)&&!present(fm.date)&&!present(fm.pubDate))missing.published++;
