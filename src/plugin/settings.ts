...
export class DeepsidianSettings extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(el).setName('本库记忆').setDesc('跨会话读取、手动编辑、模型提案与人工确认；闲时整理尚未启用。').addButton(b=>b.setButtonText('管理记忆').onClick(()=>this.plugin.openMemory())).addButton(b=>b.setButtonText('编辑规则').onClick(()=>this.plugin.openMemory('rules'))).addButton(b=>b.setButtonText('从会话提炼').onClick(()=>this.plugin.openOrganizer()));
