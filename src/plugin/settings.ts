...
export class DeepsidianSettings extends PluginSettingTab {
  ...
  display() {
    const el = this.containerEl; for(const section of Array.from(el.querySelectorAll<HTMLDetailsElement>('[data-settings-section]')))this.expanded.set(section.dataset.settingsSection!,section.open); el.empty(); el.addClass('ds-settings'); el.createEl('h2', { text: 'Deepsidian' });
