...
export class DeepsidianSettings extends PluginSettingTab {
  ...
  display() {
    ...
    new Setting(el).setName('允许 AI 管理记忆').setDesc('默认关闭。开启后，你可以直接说“请记住”“更正”或“忘记”，AI会调用工具保存变更，无需再次确认。需同时开启本库及本会话记忆读取、会话贡献。只处理当前消息的明确要求；可在记忆管理的维护页撤销最近操作。切换会重新加载工具，下一次请求生效。').addToggle(t=>t.setValue(this.plugin.state.settings.manageMemory).onChange(async value=>{try{await this.plugin.setManageMemory(value);}catch(error){t.setValue(this.plugin.state.settings.manageMemory);new Notice(String(error));}}));
