...
export class SetupModal extends Modal {
  ...
  onOpen() {
    ...
    step('1 · 安装 Node.js', '安装 Node.js 24 或更新版本。安装后重启 Obsidian，让应用读取新的 PATH。').createEl('a', { text: 'Node.js 官方下载', href: 'https://nodejs.org/en/download' });
    ...
    for (const [key, name, hint] of [
      ['packageRoot', 'DSH 包目录', '通常留空自动检测；可用 npm root -g 定位'],
      ['nodePath', 'Node 可执行文件', '通常留空；Windows 可用 where.exe node 定位'],
